...
public class RebuildConfiguration extends KVStorage {
     ...
     * 邮件账号
     *
     * @return returns [MailUser, MailPassword, MailAddr, MailName, MailCc, MailBcc,
     *         MailSmtpServer]
     */
    public static String[] getMailAccount() {
        String[] set = getsNoUnset(false,
                ConfigurationItem.MailUser, ConfigurationItem.MailPassword, ConfigurationItem.MailAddr,
    ...
    public static String getHomeUrl(String path) {
        ...
        if (path != null)
            homeUrl = joinPath(homeUrl, path);
        else if (!homeUrl.endsWith("/"))
            homeUrl += "/";
        return homeUrl;
    }
    ...
    public static String getMobileUrl(String path) {
        ...
        mobileUrl = "/";
        if (path != null)
            mobileUrl = joinPath(mobileUrl, path);
