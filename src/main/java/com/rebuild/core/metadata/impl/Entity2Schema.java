...
public class Entity2Schema extends Field2Schema {
    ...
    public String createEntity(String entityName, String entityLabel, String comments, String mainEntity,
            boolean haveNameField, boolean haveSeriesField) {
        if (!License.isRbvAttached() && MetadataHelper.getEntities().length >= 100) {
            throw new NeedRbvException(Language.L("实体数量超出 限制"));
        }
        ...
        try {
            ...
            if (haveNameField) {
                createUnsafeField(
                        tempEntity, nameFiled, Language.L("%s名称", entityLabel), DisplayType.TEXT, false, true, true,
                        true, true, null, null, null, null, null);
            }
            if (haveSeriesField) {
                createUnsafeField(
                        tempEntity, entityName + "No", Language.L("%s编号", entityLabel), DisplayType.SERIES, false,
                        false, false, false, true, null, null, null, null, null);
            }

            createBuiltinField(tempEntity, EntityHelper.CreatedBy, Language.L("创建人"), DisplayType.REFERENCE, null,
                    "User", null);
            createBuiltinField(tempEntity, EntityHelper.CreatedOn, Language.L("创建时间"), DisplayType.DATETIME, null, null,
                    null);
            createBuiltinField(tempEntity, EntityHelper.ModifiedBy, Language.L("修改人"), DisplayType.REFERENCE, null,
                    "User", null);
            createBuiltinField(tempEntity, EntityHelper.ModifiedOn, Language.L("修改时间"), DisplayType.DATETIME, null,
                    null, null);
            ...
            if (isDetail) {
                ...
                createBuiltinField(tempEntity, mainForeign, mainLabel, DisplayType.REFERENCE, Language.L("引用主记录"),
                        mainEntity, CascadeModel.Delete);
            } else {
                // 助记码/搜索码
                createUnsafeField(
                        tempEntity, EntityHelper.QuickCode, Language.L("助记码"), DisplayType.TEXT, true, false, false,
                        true, false, null, null, null, null, null);

                createBuiltinField(tempEntity, EntityHelper.OwningUser, Language.L("所属用户"), DisplayType.REFERENCE, null,
                        "User", null);
                createBuiltinField(tempEntity, EntityHelper.OwningDept, Language.L("所属部门"), DisplayType.REFERENCE, null,
                        "Department", null);
            }
     ...
     * @return
     */
    private Field createBuiltinField(Entity entity, String fieldName, String fieldLabel, DisplayType displayType,
            String comments,
            String refEntity, CascadeModel cascade) {
        return createUnsafeField(
                entity, fieldName, fieldLabel, displayType, false, false, false, true, true, comments, refEntity,
                cascade, null, null);
    }
    ...
    protected int genEntityTypeCode() {
        ...
        if (s > starts) {
            for (int typeCode = s; typeCode < 999; typeCode++) {
                if (!MetadataHelper.containsEntity(typeCode))
                    return typeCode;
            }
        }
        ...
        int typeCode = mTypeCode == null || ObjectUtils.toInt(mTypeCode[0]) == 0
                ? 999
                : (ObjectUtils.toInt(mTypeCode[0]) - 1);
